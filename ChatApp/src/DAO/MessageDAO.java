...
//    }
    ...
    public static List<Message> getGroupChatHistory(String user) {
        List<Message> list = new ArrayList<>();
        String sql = "SELECT * FROM messages WHERE receiver = 'all' AND type = 'text' ORDER BY timestamp ASC";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public static List<Message> getPrivateChatHistory(String user1, String user2) {
        List<Message> list = new ArrayList<>();
        String sql = "SELECT * FROM messages WHERE " +
                "((sender=? AND receiver=?) OR (sender=? AND receiver=?)) " +
                "ORDER BY timestamp ASC";
